const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/index-D6Fm9ZNQ.js","assets/index-Cu7OsvRD.css"])))=>i.map(i=>d[i]);
import{C as i,a as g,f as m,e as o,D as u,E as e,i as f,b,n as t,F as r,p as c,_ as y,j as C}from"./index-D6Fm9ZNQ.js";const h={async mount(a){const s=C(),l=i.depth>1;a.innerHTML=`
      ${g({title:"Settings",showBack:l})}
      <div class="scroll-view"><div class="scroll-content">
        ...
        <div class="card" style="margin:var(--sp-lg)">
          <div class="card-body" style="display:flex;align-items:center;gap:var(--sp-lg)">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="margin-top:var(--sp-xs)">${u(s?.userRole||"owner")}</div>
            </div>
          </div>
        </div>
        ...
        <div style="padding:0 var(--sp-lg) var(--sp-lg);display:flex;flex-direction:column;gap:var(--sp-sm)">
          <button class="btn btn-secondary btn-full" id="btn-logout">${f("logout",18)} Sign Out</button>
          <button class="btn btn-danger btn-full btn-sm" id="btn-delete" style="margin-top:var(--sp-md)">Delete Account & Data</button>
        </div>
      </div></div>`,b(a,{onBack:()=>i.depth>1?t.pop():t.switchTab("dashboard")}),a.querySelectorAll("[data-action]").forEach(n=>{n.addEventListener("click",()=>t.push(n.dataset.action))}),a.querySelector("#btn-logout").addEventListener("click",async()=>{await r({title:"Sign Out",message:"Are you sure you want to sign out?",confirmText:"Sign Out"})&&c()}),a.querySelector("#btn-delete").addEventListener("click",async()=>{if(!await r({title:"Delete Account",message:"This will permanently delete your account and all gym data. This cannot be undone.",confirmText:"Delete Permanently",destructive:!0}))return;const{deleteAccount:d}=await y(async()=>{const{deleteAccount:v}=await import("./index-D6Fm9ZNQ.js").then(p=>p.P);return{deleteAccount:v}},__vite__mapDeps([0,1]));await d(),c()})}};export{h as default};
