import{i,e as l,f as _,g as q,n as s,j as k,k as u,o as R,p as S,q as z,t as o,u as c,v as n,w as y,x as T,y as A}from"./index-D6Fm9ZNQ.js";const C={async mount(r){const t=k(),v=t?.tenantName||"Your Gym",p=t?.userName||"";r.innerHTML=`
      ...
      <div class="scroll-view" id="dash-scroll">
        ${q()}
      </div>`,r.querySelector("#dash-notifications-btn")?.addEventListener("click",()=>s.push("notifications")),r.querySelector("#dash-settings-btn")?.addEventListener("click",()=>s.push("settings")),await x(r)}};async function x(r){const t=r.querySelector("#dash-scroll");if(!t)return;const p=k()?.userName||"",[d,b,g]=await Promise.all([u("/api/mobile/v1/dashboard"),u("/api/mobile/v1/renewals/upcoming"),u("/api/mobile/v1/payments?page_size=5")]);if(!d.ok){t.innerHTML=R(d.error.message,"dash-retry"),t.querySelector("#dash-retry")?.addEventListener("click",()=>x(r)),d.error.status===401&&S();return}const e=d.data,m=b.ok?b.data.members||[]:[],f=g.ok?g.data.payments||[]:[];t.innerHTML=`<div class="scroll-content" style="padding:0">
    ...
    <!-- Upcoming Renewals -->
    ${m.length>0?`
      ${y("Upcoming Renewals","View All","view-all-renewals")}
    ...
    <!-- Recent Payments -->
    ${f.length>0?`
      ${y("Recent Payments","View All","view-all-payments")}
      <div class="card" style="margin:0 var(--sp-lg) var(--sp-lg)">
